...
class DescriptionEditor extends React.Component {
  ...
  static splitDescriptionIntoSections(operator) {
    let description = _.get(operator, 'spec.description', '');
  ...
  static validateMarkdown(markdown) {
    ...
    return containsH1 ? 'Heading level 1 is discouraged in the description as it collides with the rest of the page. Please use heading level 2 or higher.' : null;
  }
  ...
  onUpdate = (type, markdown) => {
    ...
    if (['application', 'about', 'prerequisites'].includes(type)) {
      ...
      const descriptioText = Object.values(description).reduce(
        (aggregator, value) => {
          return aggregator + (value.endsWith('\n') ? value : value + '\n');
        }, '');
